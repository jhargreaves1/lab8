...
void getData(vector <SBSIT> *dataArray);
...
int main(int argc, char *argv[])
{
  ...
  if ( argc != 3 && argc > 1 )
    {
      cout<<argc<<" ";
      cerr<< "Usage:" << argv[0] << "Please enter tolerance i.e 0.005 \n";
      return 1;
    
    }

  //indicates if test completes succesfully
  int testResult =0;
  ...
  if ( argc == 3) {
    ...
    cout<< "\nUnit test for SBSI function... "<<endl;
    cout <<"tolerance: "<<tolerance<<endl;
    int errorCount = 0;
    vector <SBSIT> dataArray;

    getData(& dataArray);
    cout.precision(10);
    cout << "H   \tWC     \tBSA   \tVTC    \tSBSI       \tFUNCTION" << endl;
    cout << "___     ___    _____    ___     _________        _________"<< endl;  
    for ( unsigned i = 0; i < dataArray.size(); i++){
      cout << dataArray[i].h <<"   \t";
      cout << dataArray[i].wc <<"    \t";
      cout << dataArray[i].bsa <<" \t";
      cout << dataArray[i].vtc <<"  \t";
      cout << dataArray[i].rslt <<"  \t";
      SBSI sbsi;
      sbsi.h = dataArray[i].h;
      sbsi.wc = dataArray[i].wc;
      sbsi.bsa = dataArray[i].bsa;
      sbsi.vtc = dataArray[i].vtc;
      float sbsi_calc = bodyShapeIndex(&sbsi); 
      cout <<sbsi_calc <<" ";
      float dif = 0;
      dif = fabs( sbsi_calc - dataArray[i].rslt);
      if (dif > tolerance){
    ...
    cout << "\nbad element count: " << errorCount << endl;
    cout << "Unit Test complete." << endl;
    //Flt val = bodyShapeIndex(&sbsi);
  }
  // end unit test ===================================
  // if test complete now exit
  if (tolerance !=0)
    return testResult;
